...
class OptionsSubState extends flixel.FlxSubState {
    public var optionOptions:Array<String> = ['Volume', 'Controls', 'How To Play'];
    ...
    public function new() {
        ...
        for(i in 0...optionOptions.length) {
            selectionText = new FlxText(50, 80 + (40 * i), 300, optionOptions[i]);
            selectionText.setFormat(null, 24, fillColor, CENTER, OUTLINE, outlineColor);
            selectionText.ID = i;
            selectionText.screenCenter(X);
            mainOptionsAssets.push(selectionText);
            add(selectionText);
        ...
        cursor = new FlxSprite(100, mainOptionsAssets[1].y + (mainOptionsAssets[1].height / 2) - 15 , Paths.image('ui/menu/pointer'));
		add(cursor);

        // creating assets for volume menu
        volumeAssets = new Array<Dynamic>();
        var selectionText = new FlxText(50, 20, 400, "Volume");
        selectionText.setFormat(null, 24, fillColor, CENTER, OUTLINE, outlineColor);
        selectionText.screenCenter(X);
        volumeAssets.push(selectionText);
        add(selectionText);

        masterSoundBar = new FlxSlider(masterSound, "x", 140, 60, 0, 100, 200, 30, 15, fillColor, outlineColor);
        masterSoundBar.nameLabel.text = 'Master Volume';
        masterSoundBar.hoverAlpha = 1;
        masterSoundBar.valueLabel.color = fillColor;
        masterSoundBar.nameLabel.setFormat(null, 16, fillColor, CENTER, OUTLINE, outlineColor);
        volumeAssets.push(masterSoundBar);
        add(masterSoundBar);

        sfxSoundBar = new FlxSlider(sfxSound, "x", 140, 120, 0, 100, 200, 30, 15, fillColor, outlineColor);
        sfxSoundBar.nameLabel.text = 'SFX Volume';
        sfxSoundBar.hoverAlpha = 1;
        sfxSoundBar.valueLabel.color = fillColor;
        sfxSoundBar.nameLabel.setFormat(null, 16, fillColor, CENTER, OUTLINE, outlineColor);
        volumeAssets.push(sfxSoundBar);
        add(sfxSoundBar);

        musicSoundBar = new FlxSlider(musicSound, "x", 140, 180, 0, 100, 200, 30, 15, fillColor, outlineColor);
        musicSoundBar.nameLabel.text = 'Music Volume';
        musicSoundBar.hoverAlpha = 1;
        musicSoundBar.valueLabel.color = fillColor;
        musicSoundBar.nameLabel.setFormat(null, 16, fillColor, CENTER, OUTLINE, outlineColor);
        ...
        var selectionText = new FlxText(50, 20, 400, "Controls");
        selectionText.setFormat(null, 24, fillColor, CENTER, OUTLINE, outlineColor);
        selectionText.screenCenter(X);
        ...
        for(i in 0...controlOptions.length) {
            ...
            selectionText = new FlxText(10, 65 + (25 * i), 240, controlOptions[i][0]);
            selectionText.setFormat(null, 18, fillColor, CENTER, OUTLINE, outlineColor);
            controlAssets.push(selectionText);
            add(selectionText);

            selectionText = new FlxText(100, 65 + (25 * i), 240, defaultKey.toString());
            selectionText.setFormat(null, 18, outlineColor, CENTER, OUTLINE, outlineColor);
            controlChoices.push(selectionText);
            controlAssets.push(selectionText);
            add(selectionText);

            selectionText = new FlxText(160, 65 + (25 * i), 240, subKey.toString());
            selectionText.setFormat(null, 18, outlineColor, CENTER, OUTLINE, outlineColor);
            controlChoices.push(selectionText);
            controlAssets.push(selectionText);
            add(selectionText);
        ...
        remapText.setFormat(null, 12, fillColor, CENTER, OUTLINE, outlineColor);
        remapText.screenCenter(X);
        remapArray.push(remapText);
        add(remapText);

        remapText = new FlxText(0, 0, Global.width, "REMAPPING ERROR");
        remapText.setFormat(null, 19, fillColor, CENTER, OUTLINE, outlineColor);
        ...
        for(i in 0...4) {
            ...
            switch(i) {
                case 0:
                    ...
                    selectionText = new FlxText(50, 100, 400, "Inspired by the coolest funkin game on NG, this special celebration of the annual tradition known as the non-denominational Tankmas is, like the event it's based off of, all for fun. That's right: there's no way to lose unless of course you go AFK. (Don't even try me I'm watching you)\nTry your best to go for the highest score - there's no health mechanics to worry about! Choose the character you want to play as and go for the highest score you can. Missed notes still deduct from your score, so do your best!");
                    selectionText.setFormat(null, 11, fillColor, CENTER, OUTLINE, outlineColor);
                ...
                case 1:
                    ...
                    selectionText = new FlxText(189, 162, 0, '${ControlsList.keys.get(Action.LEFT)[0]}\n${ControlsList.keys.get(Action.LEFT)[1]}');
                    selectionText.setFormat(null, 10, fillColor, CENTER, OUTLINE, outlineColor);
                    ...
                    selectionText = new FlxText(259, 162, 0, '${ControlsList.keys.get(Action.DOWN)[0]}\n${ControlsList.keys.get(Action.DOWN)[1]}');
                    selectionText.setFormat(null, 10, fillColor, CENTER, OUTLINE, outlineColor);
                    selectionText.x -= (selectionText.width / 2);
                    playPageArray.push(selectionText);
                    add(selectionText);

                    selectionText = new FlxText(294, 162, 0, '${ControlsList.keys.get(Action.RIGHT)[0]}\n${ControlsList.keys.get(Action.RIGHT)[1]}');
                    selectionText.setFormat(null, 10, fillColor, CENTER, OUTLINE, outlineColor);
                    selectionText.x -= (selectionText.width / 2);
                    playPageArray.push(selectionText);
                    add(selectionText);
                ...
                case 2:
                    ...
                    for(key in Options.scoreMap.keys()) {
                        ...
                        selectionText.setFormat(null, 12, fillColor, CENTER, OUTLINE, outlineColor);
                        selectionText.x = infoGraphic.x + (infoGraphic.width / 2) - (selectionText.width / 2);
                        playPageArray.push(selectionText);
                        add(selectionText);
                    }
                    
                    selectionText = new FlxText(50, 220, 400, "-10 point deduction per missed note!");
                    selectionText.setFormat(null, 12, fillColor, CENTER, OUTLINE, outlineColor);
                    selectionText.screenCenter(X);
                ...
                case 3:
                    ...
                    selectionText.setFormat(null, 7, fillColor, CENTER, OUTLINE, outlineColor);
                    selectionText.screenCenter(X);
                    playPageArray.push(selectionText);
                    add(selectionText);
        ...
        selectionText = new FlxText(0, 0, 440, 'Return to Options: ${GameGlobal.getSelectionInputs(null, Action.B)} - Next Page: ${GameGlobal.getSelectionInputs(null, Action.A) + '/' + GameGlobal.getSelectionInputs(null, Action.RIGHT)} - Prev Page: ${GameGlobal.getSelectionInputs(null, Action.LEFT)}');
        selectionText.setFormat(null, 10, FlxColor.WHITE, CENTER, OUTLINE, outlineColor);
        selectionText.screenCenter(X);
        selectionText.y = 250 - selectionText.height;
        ...
        for (txt in controlChoices)
            {
                if(txt.ID == controlChoice) {
                    txt.setFormat(null, 18, fillColor, CENTER, OUTLINE, outlineColor);
                } else {
                    txt.setFormat(null, 18, outlineColor, CENTER, OUTLINE, outlineColor);
                }
            }
        ...
        if(Controls.justPressed.ANY && !remap) {
            ...
            if(Controls.justPressed.LEFT) {
                if (inVolume && volAdj) {
                    ...
                    if (playPage == 1) playPage = 4 else playPage--;
            ...
            if(Controls.justPressed.RIGHT) {
                if (inVolume && volAdj) {
                    ...
                    for(asset in playAssets[playPage]) asset.kill();
                    if (playPage == 4) playPage = 1 else playPage++;
                    for(asset in playAssets[playPage]) asset.revive();
                }
            };
            if(inVolume || volAdj) {
