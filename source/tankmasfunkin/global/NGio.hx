...
class NGio
{
	...
	//this was causing problems for me in all honesty

	inline static var DEBUG_SESSION = #if NG_DEBUG true #else false #end;

	public function new(api:String, encKey:String, ?sessionId:String = null)
	{

		trace("connecting to newgrounds");

		NG.create(api, sessionId, DEBUG_SESSION);

		var previousId:String = null;
		
		if (FlxG.save.data.ngioSessionId != null)
			{
				previousId = FlxG.save.data.ngioSessionId;
			}

		NG.createAndCheckSession(api, DEBUG_SESSION, previousId);
	...
	function onNGBoardsFetch(outcome:Outcome<CallError>)
	{
		...
		var board = NG.core.scoreBoards.get(12452);// ID found in NG project view
	...
	inline static public function checkMedalUnlock(id:Int):Bool
		{
			var out:Bool = false;
			if (isLoggedIn)
			{
				var medal = NG.core.medals.get(id);
				if (medal.unlocked) out = true;
			}
			return out;
		}
}